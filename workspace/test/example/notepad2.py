...
# 학습에 필요한 실제 데이터를 로드. 결과값을 one hot 인코딩 형태로 가져옴.
...
# 학습 모델 정의 : 3개의 Convolution Layer와 2개의 Full Connected Layer로 정의함.
...
# 왜 (x,x,32)이냐하면 max pool 결과가 (x,x,32)이기 때문.
...
# 다음 레이어가 Fullly Connected Layer 이기 때문에, L3의 output을 1차원 배열로 변경.
L3_flat = tf.reshape(L3, [-1, 4 * 4 * 32])

# 기존 DNN 모델 사용(FC)
# 입력 데이터는 전단의 출력과 동일한 4 * 4 * 32 종류이고, 출력은 32개로 설정.
# 가중치 초기화에 Xavier 사용.
...
for epoch in range(training_cnt):
    avg_cost = 0
    # 학습 데이터가 크므로 전체데이터를 분할하여 학습할 계획.
    total_batch = int(mnist.train.num_examples / batch_size)

    for i in range(total_batch):
        # 1회에 학습할 데이터 추출 (자동으로 다음 데이터를 이어서 가져옴)
        batch_xs, batch_ys = mnist.train.next_batch(batch_size)
        # 텐서 플로우 변수에 학습 데이터 연결.
...
# [출처] [머신러닝] 이미지 인식 CNN (Convolutional Neural Network) 파이썬 강의 강좌 예제|작성자 idrukawa
